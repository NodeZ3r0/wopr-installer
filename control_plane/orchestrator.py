    ...
    async def _provision_vps(self, job: ProvisioningJob) -> bool:
        ...
        try:
            ...
            provision_config = ProvisionConfig(
                ...
                plan_id=self._get_plan_for_tier(job.storage_tier, job.provider_id, job.bundle),
                ssh_keys=[],
                image="debian-12",
                user_data=user_data,
...
{bootstrap_yaml}
  ...
  - path: /opt/wopr/install.sh
    ...
    content: |
      ...
      WOPR_INSTALL_DIR="/opt/wopr/installer"
      ORCHESTRATOR_URL="https://orc.{self.WOPR_DOMAIN}"
      ...
      # Download installer tarball from orchestrator
      echo "Downloading installer from $ORCHESTRATOR_URL..."
      mkdir -p "$WOPR_INSTALL_DIR"
      curl -fsSL "$ORCHESTRATOR_URL/api/installer/latest.tar.gz" | tar xz -C "$WOPR_INSTALL_DIR"

      # Run the installer
      cd "$WOPR_INSTALL_DIR"
      if [ -f scripts/scripts/wopr_install.sh ]; then
        chmod +x scripts/scripts/wopr_install.sh
        bash scripts/scripts/wopr_install.sh --domain {job.wopr_subdomain}.{self.WOPR_DOMAIN} --non-interactive --confirm-all
      else
...
""".replace('\r\n', '\n').replace('\r', '\n')
    ...
    def _get_plan_for_tier(self, storage_tier: int, provider_id: str, bundle_id: str = None) -> str:
        """Map storage tier AND bundle complexity to a provider plan ID.
        
        Bundle complexity determines minimum server size:
        - Enterprise (48+ modules) -> VERY_HIGH (cpx51/32GB)
        - Professional/SmallBusiness (22+ modules) -> HIGH (cpx42/16GB)
        - Developer/Creator (10-20 modules) -> MEDIUM (cpx32/8GB)
        - Starter/Family/Micro (<10 modules) -> LOW (cpx22/4GB)
        ...
        bundle_tier = BUNDLE_COMPLEXITY.get(bundle_id, 1) if bundle_id else 1
        
        # Use the HIGHER of bundle complexity or storage tier
        effective_tier = max(bundle_tier, storage_tier)
        
        # Clamp to valid range 1-4
        effective_tier = max(1, min(4, effective_tier))
        
        # Provider plans: tier 1=LOW, 2=MEDIUM, 3=HIGH, 4=VERY_HIGH
        tier_plans = {
            "hetzner": {1: "cpx22", 2: "cpx32", 3: "cpx42", 4: "cpx51"},
            "digitalocean": {1: "s-2vcpu-4gb", 2: "s-4vcpu-8gb", 3: "s-8vcpu-16gb", 4: "s-8vcpu-32gb"},
            "linode": {1: "g6-standard-2", 2: "g6-standard-4", 3: "g6-standard-6", 4: "g6-standard-8"},
            "ovh": {1: "B2-7", 2: "B2-15", 3: "B2-30", 4: "B2-60"},
            "upcloud": {1: "2xCPU-4GB", 2: "4xCPU-8GB", 3: "6xCPU-16GB", 4: "8xCPU-32GB"},
        }
        provider_plans = tier_plans.get(provider_id, tier_plans["hetzner"])
        return provider_plans.get(effective_tier, provider_plans.get(1, "cpx22"))
