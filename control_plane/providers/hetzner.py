...
class HetznerProvider(WOPRProviderInterface):
    ...
    def get_recommended_plan(self, tier: ResourceTier) -> Optional[Plan]:
        ...
        """
        recommendations = {
            ResourceTier.LOW: "cpx22",
            ResourceTier.MEDIUM: "cpx32",
            ResourceTier.HIGH: "cpx42",
            ResourceTier.VERY_HIGH: "cpx51",
        }
    ...
    def provision(self, config: ProvisionConfig) -> Instance:
        ...
        try:
            ...
            if config.wopr_customer_email:
                labels["wopr-email"] = config.wopr_customer_email.replace("@", "_at_").replace(".", "_")[:63]
