...
def create_dashboard_app(
    ...
    @app.post("/api/v1/provisioning/{job_id}/callback")
    async def provisioning_callback(job_id: str, request: Request):
        """
        Callback endpoint for beacons to report installation progress.
        
        This replaces SSH/HTTP polling - the beacon's cloud-init script
        POSTs progress updates here using a pre-shared token.
        """
        from control_plane.callbacks import callback_manager
        ...
        success = await callback_manager.handle_callback(
            job_id=job_id,
            token=token,
            stage=stage,
            progress=progress,
        ...
        # Also update the in-memory job store for SSE streaming
        if job_id in _provisioning_jobs:
            # Map callback stage to step number
            stage_to_step = {
                "boot": 1,
                "packages": 1,
                "wopr_download": 2,
            ...
            step = stage_to_step.get(stage, 0)
            
            _provisioning_jobs[job_id].update({
                "step": step,
                "progress": progress,
                "current_module": module,
                "status": "error" if stage == "error" else ("complete" if stage == "complete" else "in_progress"),
                "error_message": error if error else None,
            })
        
        logger.info(f"Callback received for job {job_id}: {stage} ({progress}%)")
        return {"status": "ok", "stage": stage}

    @app.get("/api/v1/provisioning/{job_id}/progress")
    async def get_provisioning_progress(job_id: str):
        """
        Get current provisioning progress for a job.
        Uses callback-based tracking instead of polling.
        """
        from control_plane.callbacks import callback_manager
        
        progress = await callback_manager.get_progress(job_id)
        if progress:
            return progress
        
        # Fallback to in-memory store
        if job_id in _provisioning_jobs:
            job = _provisioning_jobs[job_id]
            return {
                "stage": job.get("status", "pending"),
                "progress": job.get("progress", 0),
                "module": job.get("current_module"),
