...
# WOPR Staff Admin (lighthouse-only, NOT for customer beacons)
register_module(Module(id="wopr-admin", name="WOPR Staff Admin", description="WOPR Staff escalations dashboard and AI remediation approval (LIGHTHOUSE ONLY)", category=ModuleCategory.SECURITY, tier=ModuleTier.LOW, container_image="wopr/support-gateway:latest", default_port=8443, sso_type=SSOType.PROXY, authentik_app_slug="support-gateway", authentik_group="wopr-staff-admin", bundles=[], dependencies=["authentik","caddy","postgresql"], subdomain="support", ram_mb=512, replaces="Manual server administration"))

# Customer support client (for customer beacons - allows receiving WOPR support)
register_module(Module(id="support-client", name="WOPR Support Client", description="Zero-trust remote support client with SSH CA - receive WOPR staff support", category=ModuleCategory.SECURITY, tier=ModuleTier.LOW, container_image="wopr/support-client:latest", default_port=8444, sso_type=SSOType.PROXY, authentik_app_slug="support-client", authentik_group="beacon-owner", bundles=["professional","small_business","enterprise"], dependencies=["authentik","caddy"], subdomain="support", ram_mb=256, replaces="Custom SSH access, VPN-only support"))
