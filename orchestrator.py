...
{bootstrap_yaml}
  ...
  - path: /opt/wopr/install.sh
    ...
    content: |
      ...
      # Run the installer
      cd "$WOPR_INSTALL_DIR"
      if [ -f scripts/wopr_install.sh ]; then
        chmod +x scripts/wopr_install.sh
        bash scripts/wopr_install.sh --domain {job.wopr_subdomain}.{self.WOPR_DOMAIN} --non-interactive --confirm-all
      else
